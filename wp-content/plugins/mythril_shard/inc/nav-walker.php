<?php
/**
 * Custom walker class to address sub-menu styling needs.
 */

/**
 * Main Menu Dropdown Compatible Walker
 */
...
/**
 * Mobile Menu Specific Navigation
 */
class Mobile_Main_Menu_Walker_Nav_Menu extends Walker_Nav_Menu {
    ...
    function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
        ...
        // Passed classes.
        $classes = empty( $item->classes ) ? array() : (array) $item->classes;
        $caret = '<svg class="icon caret-right"><use class="inner-icon" xlink:href="#caret-right"></use></svg>';

        $class_names = esc_attr( implode( ' ', apply_filters( 'nav_menu_css_class', array_filter( $classes ), $item ) ) );
        $showcaret = strpos($class_names, 'menu-item-has-children') > -1 ? $caret : '';
        ...
        if($depth == 0){ 
            ...
            $item_output = sprintf( '%1$s<a%2$s><span>%3$s%4$s%5$s %7$s</span></a>%6$s',
                $args->before,
                $attributes,
                $args->link_before,
            ...
            $attributes = ! empty( $item->target )     ? ' target="' . esc_attr( $item->target     ) .'"' : '';
            $attributes .= ! empty( $item->url )        ? ' href="'   . esc_attr( $item->url        ) .'"' : '';
            $attributes .= ' class="sub-menu-text "';
            ...
            $current = strpos($class_names, 'current-menu-item') > 0  ? 1 : '';
            $output .= $indent . '<li class="sub-text" data-current="'.$current.'">';
