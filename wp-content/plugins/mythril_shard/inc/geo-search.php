...
add_filter('posts_fields','mha_s2s_handle_geo_select', 10, 2);


function mha_s2s_handle_geo_joins($join_clause, $query) {
    if(isset($query->get("geo_query")["latitude"]) && isset($query->get("geo_query")["latitude"])){
        $join_clause .= "       
            LEFT JOIN wp_postmeta lats
                ON wp_posts.id = lats.post_id
                AND lats.meta_key LIKE 'location_%_latitude'
                
            LEFT JOIN wp_postmeta lngs
                ON wp_posts.id = lngs.post_id
                AND lngs.meta_key LIKE 'location_%_longitude'
        ";
    }
	return $join_clause;	
}

add_filter('posts_join_paged','mha_s2s_handle_geo_joins', 10, 2);


function mha_s2s_handle_geo_where($where_clause, $query){
...
add_filter("posts_where", "mha_s2s_handle_geo_where", 10, 2);


function mha_s2s_handle_geo_orderby($orderby_clause, $query) {
    if(isset($query->get("geo_query")["latitude"]) && isset($query->get("geo_query")["latitude"])){
...
add_filter('posts_orderby', 'mha_s2s_handle_geo_orderby', 10, 2);